...
The fix is basically to put the `HP Spectre x360 Convertible 14-ea1xxx` sound driver subsystem_id which happen to be 0x103c89db within the `patch_realtek.c`, and yes i also added one with 0x103c0000 because for some reason that is what is shown within `dmesg` with unknown reason (at least for me)
