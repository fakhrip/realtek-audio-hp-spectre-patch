			    ...
			    bool match_all_pins)
{
	...
	codec_info(codec, "%s: finding pin fixup\n",
	...
	for (pq = pin_quirk; pq->subvendor; pq++) {
		...
		if (pin_config_match(codec, pq->pins, match_all_pins)) {
			...
			name = pq->name;
#endif
			codec_info(codec, "%s: picked fixup %s (pin match) oh yeahhh\n",
			...
			const struct hda_fixup *fixlist)
{
	...
	codec_info(codec, "%s: finding fixup\n",
		   codec->core.chip_name);
	...
	if (codec->modelname && models) {
		while (models->name) {
			if (!strcmp(codec->modelname, models->name)) {
				id = models->id;
				name = models->name;
				codec_info(codec, "%s: picked fixup %s (model specified) oh yeahhh\n",
 ...
 found_device:
	...
	name = q->name;
#endif
	codec_info(codec, "%s: picked fixup %s for %s %04x:%04x oh yeahhh\n",
		   codec->core.chip_name, name ? name : "",
